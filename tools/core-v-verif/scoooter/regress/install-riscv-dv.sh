...
export TB_PATH=$ROOT_PROJECT/scoooter/tb/core
export TESTS_PATH=$ROOT_PROJECT/scoooter/tests
...
mkdir -p scoooter/sim
if ! [ -d scoooter/sim/dv ]; then
  git clone $DV_REPO -b $DV_BRANCH scoooter/sim/dv
  cd scoooter/sim/dv; git checkout $DV_HASH;
  if [[ -n "$DV_PATCH" && -f "$DV_PATCH" ]]; then
    git apply "$DV_PATCH"
  fi
  cd -
  # install riscv-dv dependencies
  cd scoooter/sim/dv; pip3 install -r requirements.txt; cd -
fi
