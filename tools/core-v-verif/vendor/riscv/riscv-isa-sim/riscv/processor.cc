...
void state_t::reset(processor_t* const proc, reg_t max_isa)
{
  ...
  if (xlen == 32) {
    ...
    mstatush = std::make_shared<rv32_high_csr_t>(proc, CSR_MSTATUSH, mstatus);
  } else {
  ...
  if (proc->extension_enabled_const(EXT_ZICNTR)) {
    ...
    time_proxy = std::make_shared<counter_proxy_csr_t>(proc, CSR_TIME, time);
  }
  if (xlen == 32) {
    csr_t_p minstreth, mcycleh;
    std::make_shared<rv32_low_csr_t>(proc, CSR_MINSTRET, minstret);
    minstreth = std::make_shared<rv32_high_csr_t>(proc, CSR_MINSTRETH, minstret);
    std::make_shared<rv32_low_csr_t>(proc, CSR_MCYCLE, mcycle);
    mcycleh = std::make_shared<rv32_high_csr_t>(proc, CSR_MCYCLEH, mcycle);
    if (proc->extension_enabled_const(EXT_ZICNTR)) {
      auto timeh = std::make_shared<rv32_high_csr_t>(proc, CSR_TIMEH, time);
      std::make_shared<counter_proxy_csr_t>(proc, CSR_INSTRETH, minstreth);
      std::make_shared<counter_proxy_csr_t>(proc, CSR_CYCLEH, mcycleh);
      std::make_shared<counter_proxy_csr_t>(proc, CSR_TIMEH, timeh);
    }
  } else {
  ...
  for (reg_t i = 3; i < N_HPMCOUNTERS + 3; ++i) {
    ...
    if (xlen == 32) {
      std::make_shared<rv32_low_csr_t>(proc, which_mevent, mevent[i - 3]);;
  ...
  std::make_shared<const_csr_t>(proc, CSR_MCOUNTINHIBIT, 0);
  if (proc->extension_enabled_const(EXT_SSCOFPMF))
    std::make_shared<scountovf_csr_t>(proc, CSR_SCOUNTOVF);
  ...
  std::make_shared<virtualized_csr_t>(proc, nonvirtual_sip, vsip);
  std::make_shared<mip_proxy_csr_t>(proc, CSR_HIP, hip_hie_accr);
  std::make_shared<mip_proxy_csr_t>(proc, CSR_HVIP, hvip_accr);
  ...
  std::make_shared<virtualized_csr_t>(proc, nonvirtual_sie, vsie);
  std::make_shared<mie_proxy_csr_t>(proc, CSR_HIE, hip_hie_accr);
  ...
  vsepc = std::make_shared<epc_csr_t>(proc, CSR_VSEPC);
  sepc = std::make_shared<virtualized_csr_t>(proc, nonvirtual_sepc, vsepc);
  auto nonvirtual_stval = std::make_shared<basic_csr_t>(proc, CSR_STVAL, 0);
  vstval = std::make_shared<basic_csr_t>(proc, CSR_VSTVAL, 0);
  stval = std::make_shared<virtualized_csr_t>(proc, nonvirtual_stval, vstval);
  ...
  std::make_shared<virtualized_csr_t>(proc, sscratch, vsscratch);
  //csrmap[CSR_VSSCRATCH] = vsscratch;
  auto nonvirtual_stvec = std::make_shared<tvec_csr_t>(proc, CSR_STVEC);
  vstvec = std::make_shared<tvec_csr_t>(proc, CSR_VSTVEC);
  stvec = std::make_shared<virtualized_csr_t>(proc, nonvirtual_stvec, vstvec);
  auto nonvirtual_satp = std::make_shared<satp_csr_t>(proc, CSR_SATP);
  vsatp = std::make_shared<base_atp_csr_t>(proc, CSR_VSATP);
  satp = std::make_shared<virtualized_satp_csr_t>(proc, nonvirtual_satp, vsatp);
  auto nonvirtual_scause = std::make_shared<cause_csr_t>(proc, CSR_SCAUSE);
  vscause = std::make_shared<cause_csr_t>(proc, CSR_VSCAUSE);
  scause = std::make_shared<virtualized_csr_t>(proc, nonvirtual_scause, vscause);
  mtval2 = std::make_shared<hypervisor_csr_t>(proc, CSR_MTVAL2);
  mtinst = std::make_shared<hypervisor_csr_t>(proc, CSR_MTINST);
  ...
  hstatus = std::make_shared<masked_csr_t>(proc, CSR_HSTATUS, hstatus_mask, hstatus_init);
  std::make_shared<const_csr_t>(proc, CSR_HGEIE, 0);
  std::make_shared<const_csr_t>(proc, CSR_HGEIP, 0);
  ...
  hcounteren = std::make_shared<masked_csr_t>(proc, CSR_HCOUNTEREN, counteren_mask, 0);
  htimedelta = std::make_shared<basic_csr_t>(proc, CSR_HTIMEDELTA, 0);
  if (xlen == 32) {
    std::make_shared<rv32_low_csr_t>(proc, CSR_HTIMEDELTA, htimedelta);
    std::make_shared<rv32_high_csr_t>(proc, CSR_HTIMEDELTAH, htimedelta);
  } else {
    //csrmap[CSR_HTIMEDELTA] = htimedelta;
  }
  htval = std::make_shared<basic_csr_t>(proc, CSR_HTVAL, 0);
  htinst = std::make_shared<basic_csr_t>(proc, CSR_HTINST, 0);
  hgatp = std::make_shared<hgatp_csr_t>(proc, CSR_HGATP);
  auto nonvirtual_sstatus = std::make_shared<sstatus_proxy_csr_t>(proc, CSR_SSTATUS, mstatus);
  vsstatus = std::make_shared<vsstatus_csr_t>(proc, CSR_VSSTATUS);
  sstatus = std::make_shared<sstatus_csr_t>(proc, nonvirtual_sstatus, vsstatus);

  dpc = std::make_shared<dpc_csr_t>(proc, CSR_DPC);
  std::make_shared<debug_mode_csr_t>(proc, CSR_DSCRATCH0);
  std::make_shared<debug_mode_csr_t>(proc, CSR_DSCRATCH1);
  dcsr = std::make_shared<dcsr_csr_t>(proc, CSR_DCSR);

  tselect = std::make_shared<tselect_csr_t>(proc, CSR_TSELECT);
  if (proc->get_cfg().trigger_count > 0) {
    std::make_shared<tdata1_csr_t>(proc, CSR_TDATA1);
    tdata2 = std::make_shared<tdata2_csr_t>(proc, CSR_TDATA2);
    std::make_shared<tdata3_csr_t>(proc, CSR_TDATA3);
    std::make_shared<tinfo_csr_t>(proc, CSR_TINFO);
  } else {
    std::make_shared<const_csr_t>(proc, CSR_TDATA1, 0);
    tdata2 = std::make_shared<const_csr_t>(proc, CSR_TDATA2, 0);
    std::make_shared<const_csr_t>(proc, CSR_TDATA3, 0);
    std::make_shared<const_csr_t>(proc, CSR_TINFO, 0);
  }
  ...
  /*csrmap[CSR_HCONTEXT]*/ auto hcontext = std::make_shared<masked_csr_t>(proc, CSR_HCONTEXT, (reg_t(1) << hcontext_length) - 1, 0);
  /*csrmap[CSR_MCONTEXT] = */mcontext = std::make_shared<proxy_csr_t>(proc, CSR_MCONTEXT, hcontext /*csrmap[CSR_HCONTEXT]*/);
  debug_mode = false;
  ...
  for (int i = 0; i < max_pmp; ++i) {
    pmpaddr[i] = std::make_shared<pmpaddr_csr_t>(proc, CSR_PMPADDR0 + i);
  }
  for (int i = 0; i < max_pmp; i += xlen / 8) {
    reg_t addr = CSR_PMPCFG0 + i / 4;
    std::make_shared<pmpcfg_csr_t>(proc, addr);
  }
